...
public class VoiceConnectionBuilder(
    public var gateway: GatewayBridge,
    ...
    private suspend fun updateVoiceState(): Pair<VoiceConnectionData, VoiceGatewayConfiguration> = coroutineScope {
        val voiceStateDeferred = async {
            withTimeoutOrNull(timeout) {
                gateway.events
                    .filterIsInstance<DiscordVoiceState>()
                    .first { it.guildId.value == guildId && it.userId == selfId }
            }
        }
        ...
        val voiceServerDeferred = async {
            withTimeoutOrNull(timeout) {
                gateway.events
                    .filterIsInstance<DiscordVoiceServerUpdateData>()
                    .first { it.guildId == guildId }
            }
        }
        ...
        val config = VoiceGatewayConfiguration(voiceServer.token, "wss://${voiceServer.endpoint?.removeSuffix("/")}/?v=4")
